...
docker run -itd -v ./360-LLaMA-Factory:/app -v ./models:/models --name llamafactory-360 --gpus=all --ipc=host hiyouga/llamafactory:latest /bin/bash
```
...
```bash
# 打包
docker commit llamafactory-360 llamafactory-360
docker save -o llamafactory-360.tar llamafactory-360
```

```bash
docker load -i llamafactory-360.tar
docker run -itd -v ./360-LLaMA-Factory:/app -v ./models:/models --name llamafactory-360 --gpus=all --ipc=host llamafactory-360 /bin/zsh
# 训练
